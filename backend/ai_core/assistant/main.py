...
# Import du client LLM natif
from backend.ai_core.native_llm_client import native_llm_client
...
class AssistantService:
    ...
    def _initialize_llm(self):
        """Initialise le client LLM Emergent ou natif"""
        try:
            # Priorité 1: Client natif avec clés API personnelles
            if native_llm_client.is_configured():
                self.llm_client = native_llm_client
                print("✅ Assistant IA initialisé avec client LLM natif")
                return
            
            # Priorité 2: Emergent LLM
            ...
            print("⚠️ Aucun LLM configuré - Mode simulation activé")
            self.llm_client = None
...
Tu as accès aux 35 services CyberSec Toolkit Pro 2025 portables incluant tous les outils d'audit, pentest, forensique, et business."""
    ...
    async def _generate_llm_response(self, messages: List[Dict]) -> Dict[str, Any]:
        """Génère une réponse via client LLM (natif ou Emergent)"""
        try:
            # Si on utilise le client natif
            if hasattr(self.llm_client, 'generate_response') and self.llm_client.is_configured():
                user_message = messages[-1]["content"]
                context = None
                
                # Extraire le contexte système s'il existe
                for msg in messages:
                    if msg["role"] == "system" and "CONTEXTE SPÉCIFIQUE" in msg["content"]:
                        context = msg["content"].split("CONTEXTE SPÉCIFIQUE: ")[1]
                        break
                
                response_content = await self.llm_client.generate_response(user_message, context)
            ...
            # Si on utilise Emergent LLM (méthode originale)
            ...
            # Fallback si aucun client configuré
            else:
                fallback_result = await self._generate_fallback_response(messages[-1]["content"])
...
{f"**Contexte spécifique :** {context}" if context else ""}"""
    ...
    async def _save_conversation(self, session_id: str, user_message: str, assistant_response: str):
        ...
        try:
            conversations = await get_collection("conversations")
            if not conversations:
                print("⚠️ Collection conversations non disponible")
                return
